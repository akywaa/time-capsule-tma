...
async function loadCapsule(id) {
    try {
        ...
        safeModel.addEventListener('load', () => overlay.classList.add('hidden'), { once: true });
        ...
        safeModel.setAttribute('src', isOpen ? modelPaths.open : modelPaths.closed);
        ...
        if (data.capsule_type === 'personal' && !isOpen) {
            const updateTimer = () => {
                ...
                if (diff <= 0 || data.is_hacked) {
                    ...
                    safeModel.setAttribute('src', mp.open);
